...
PRODUCT_TYPES = {
    ...
    "gown": ["gown", "gowns"],
    "co-ord": [
        "co-ord", "co ord", "coord", "coordset", "coordsets",
        "codeset", "codesets", "co-ord set", "co-ord sets",
...
COLORS = [
    "mustard yellow",  # keep longer phrase first
    "black", "white", "blue", "red", "pink", "green", "beige",
    "brown", "grey", "gray", "purple", "yellow", "orange",
    "maroon", "navy"
]

MATERIALS = [
    "cotton", "silk", "linen", "polyester", "denim", "rayon",
    "georgette", "chiffon", "satin", "wool", "crepe"
]
...
def style_tips_for_product(product, intent: Optional[str] = None) -> List[str]:
    ...
    color = (getattr(product, "color", "") or "").lower()
    material = (getattr(product, "material", "") or "").lower()

    # Intent-based
    if intent == "office":
        tips += [
            "Pair with loafers or block heels for a clean formal look.",
            "Carry a structured tote bag and minimal jewelry like studs and a watch.",
            "Layer with a blazer or long shrug for a professional finish."
        ]
    elif intent == "party":
        tips += [
            "Add statement earrings and a clutch to elevate the outfit.",
            "Wear heels or strappy sandals and go for a sleek hairstyle.",
            "Add a bold lipstick or winged eyeliner for a party-ready look."
        ]
    elif intent == "casual":
        tips += [
            "Style with sneakers or flats for comfort.",
            "Add a sling bag and light accessories like hoops or a bracelet.",
            "Keep makeup minimal and hair open or in a messy bun."
        ]
    elif intent == "sports":
        tips += [
            "Wear trainers and keep accessories minimal.",
            "Go for a ponytail or bun and add a cap if needed.",
            "Choose breathable layers and a lightweight bag."
        ]

    # Material-based
    if "cotton" in material or "linen" in material:
        tips.append("Cotton or linen looks best with simple accessories and natural makeup.")
    if "silk" in material or "satin" in material:
        tips.append("Silk or satin looks premium, so style it with heels and metallic accessories.")

    # Color-based
    if "black" in color:
        tips.append("Black pairs beautifully with gold or silver jewelry and bold lipstick.")
    if "white" in color or "beige" in color:
        tips.append("Light shades look great with nude heels and pastel bags.")
    if "red" in color:
        tips.append("Keep accessories neutral so red remains the highlight.")
    ...
    # Type hints
    if "kurta" in name:
        tips.append("Complete it with palazzo or straight pants and juttis for an elegant look.")
    if "dress" in name:
        tips.append("Add a belt to define the waist, and a denim jacket also works nicely.")
    if "co-ord" in name or "coord" in name or "set" in name:
        tips.append("Co-ord sets look best with clean accessories and matching footwear.")
...
def recommend_products(ProductModel, parsed: ParsedQuery, limit: int = 6):
    ...
    - Applies price filters first
    - Uses soft matching for intent
    - Searches across name, description, category, color, material, product_type
    - Avoids over-strict AND matching on every token
    """
    qs = ProductModel.objects.all()

    # ---------- hard filters ----------
    if parsed.min_price is not None and model_has_field(ProductModel, "price"):
        qs = qs.filter(price__gte=parsed.min_price)

    if parsed.max_price is not None and model_has_field(ProductModel, "price"):
        qs = qs.filter(price__lte=parsed.max_price)

    if parsed.color:
        color_q = Q()
        if model_has_field(ProductModel, "color"):
            color_q |= Q(color__icontains=parsed.color)
        color_q |= Q(name__icontains=parsed.color)
        if model_has_field(ProductModel, "description"):
            color_q |= Q(description__icontains=parsed.color)
        qs = qs.filter(color_q)

    if parsed.material:
        material_q = Q()
        if model_has_field(ProductModel, "material"):
            material_q |= Q(material__icontains=parsed.material)
        material_q |= Q(name__icontains=parsed.material)
        if model_has_field(ProductModel, "description"):
            material_q |= Q(description__icontains=parsed.material)
        qs = qs.filter(material_q)

    if parsed.product_type:
        type_q = Q()

        if model_has_field(ProductModel, "product_type"):
            type_q |= Q(product_type__icontains=parsed.product_type)

        type_q |= Q(name__icontains=parsed.product_type)

        if model_has_field(ProductModel, "description"):
            type_q |= Q(description__icontains=parsed.product_type)

        if model_has_field(ProductModel, "category"):
            type_q |= Q(category__name__icontains=parsed.product_type)

        # aliases for co-ord
        if parsed.product_type == "co-ord":
            aliases = [
                "co-ord", "co ord", "coord", "coordset", "coordsets",
                "codeset", "codesets", "set"
            ]
            for alias in aliases:
                type_q |= Q(name__icontains=alias)
                if model_has_field(ProductModel, "description"):
                    type_q |= Q(description__icontains=alias)
                if model_has_field(ProductModel, "product_type"):
                    type_q |= Q(product_type__icontains=alias)
                if model_has_field(ProductModel, "category"):
                    type_q |= Q(category__name__icontains=alias)
    ...
    if parsed.intent:
        ...
        if model_has_field(ProductModel, "category"):
            intent_q |= Q(category__name__icontains=parsed.intent)

        intent_q |= Q(name__icontains=parsed.intent)

        if model_has_field(ProductModel, "description"):
            intent_q |= Q(description__icontains=parsed.intent)

        matched = qs.filter(intent_q)
        if matched.exists():
            qs = matched

    # ---------- fallback keyword search ----------
    words = re.findall(r"[a-zA-Z\-]+", parsed.raw.lower())
    ignore_words = {
        "under", "below", "less", "than", "above", "more", "over",
        "need", "want", "show", "me", "for", "with", "wear", "outfit",
        "something", "please"
    }
    ...
    if words:
        ...
        for w in words:
            keyword_q |= Q(name__icontains=w)

            if model_has_field(ProductModel, "description"):
                keyword_q |= Q(description__icontains=w)

            if model_has_field(ProductModel, "color"):
                keyword_q |= Q(color__icontains=w)

            if model_has_field(ProductModel, "material"):
                keyword_q |= Q(material__icontains=w)

            if model_has_field(ProductModel, "product_type"):
                keyword_q |= Q(product_type__icontains=w)

            if model_has_field(ProductModel, "category"):
                keyword_q |= Q(category__name__icontains=w)

        fallback_qs = qs.filter(keyword_q).distinct()
        if fallback_qs.exists():
            qs = fallback_qs

    return qs.distinct().order_by("-id")[:limit]